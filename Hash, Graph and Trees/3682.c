...
if (arvore == NULL)
        return true;
    if (arvore->esq != NULL && (arvore->esq->item.chave >= arvore->item.chave || !eBinariaBusca(arvore->esq)))
        return false;
    if (arvore->dir != NULL && (arvore->dir->item.chave <= arvore->item.chave || !eBinariaBusca(arvore->dir)))
        return false;
    return true;
