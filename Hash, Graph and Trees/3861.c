...
bool haDuplicadosNaSubarvore(ARVORE_BINARIA *arvore, TIPOCHAVE chave) {
    if (arvore == NULL)
        return false;

    if (arvore->item.chave == chave)
        return true;

    return haDuplicadosNaSubarvore(arvore->esq, chave) || haDuplicadosNaSubarvore(arvore->dir, chave);
}
